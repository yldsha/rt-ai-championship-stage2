...
def main() -> int:
    parser = argparse.ArgumentParser(description="CodeLens RAG — Precision@5 scorer")
    parser.add_argument(
        "--predictions",
        required=True,
    ...
    parser.add_argument(
        "--questions", required=True, help="Path to eval_questions.json (ground-truth)"
    )
    ...
    for entry in predictions_raw:
        if not isinstance(entry, dict):
            print(
                "ERROR: each prediction entry must be a JSON object.", file=sys.stderr
            )
            return 1
    ...
    for qid, q in gt.items():
        ...
        if qid not in pred_index:
            print(f"WARNING: no prediction found for {qid}. Scoring as 0.")
