...
def ask_llm(question: str, chunks: list[dict], timeout: int = 30) -> str:
    ...
    if not GROQ_API_KEY:
        return (
            ...
            "либо впиши его напрямую в src/ui/llm.py"
        )
...
Отвечай на том языке, на котором задан вопрос (если на русском — отвечай на русском, если на английском — отвечай на английском). Будь вежлив, краток и по делу. ВАЖНО: если переданные фрагменты кода НЕ относятся напрямую к вопросу (например, вопрос про функциональность, которой в коде нет — кэширование, OAuth, WebSocket, rate limiting и т.п.) — честно скажи, что в найденном коде такой функциональности нет, 
и НЕ пытайся притянуть смежные понятия (например логирование — это не кэширование, аутентификация — это не OAuth). Не наводи ложные связи между разными техническими концепциями.
